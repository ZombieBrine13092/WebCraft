...
{
	if ( this.log ) this.log( "IP " + this.getIp(socket) + " connected to the server." );

	// Check if a slot limit is active
	if ( this.maxSlots != null && this.usedSlots == this.maxSlots ) {
		this.kick( socket, "The server is out of slots!" );
		return;
	}
	...
	{
		this.kick( socket, "You cannot use multiple clients on the same IP address!" );
		return;
	}
